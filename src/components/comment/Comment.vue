<template>
	<!--评论列表-->
	<div>
		<CommentForm v-if="parentCommentId===null"/>
		...
		<div class="comment" v-for="comment in comments" :key="comment.id">
			...
			<div class="content">
				...
				<div class="ui black left pointing label" v-if="comment.is_admin">{{ $store.state.siteInfo.commentAdminFlag }}</div>
				<div class="metadata">
					<strong class="date">{{ comment.create_time | dateFormat('YYYY-MM-DD HH:mm') }}</strong>
				</div>
				<el-button size="mini" type="primary" @click="setReply(comment.id)">回复</el-button>
				<div class="text" v-html="comment.content"></div>
			</div>
			<!-- 回复部分 -->
			<div class="comments" v-if="comment.sub_comments.length>0">
				<div class="comment" v-for="reply in comment.sub_comments" :key="reply.id">
					...
					<div class="content">
						...
						<div class="ui black left pointing label" v-if="reply.is_admin">{{ $store.state.siteInfo.commentAdminFlag }}</div>
						<div class="metadata">
							<strong class="date">{{ reply.create_time | dateFormat('YYYY-MM-DD HH:mm') }}</strong>
						</div>
            <el-button size="mini" type="primary" @click="setReply(reply.id)">回复</el-button>
						<div class="text">
							<a :href="`#comment-${reply.parent_comment.id}`">@{{ reply.parent_comment.nickname }}</a>
