				...
				<div class="moment" v-for="(moment,index) in momentList" :key="index">
					...
					<div class="ui card">
						<div class="content m-top">
							...
							<span class="right floated">{{ moment.create_time | dateFromNow }}</span>
						</div>

						<div class="content typo" :class="{'privacy':!moment.published}" v-viewer v-if="!moment.published" >此条为私密动态，暂不公开！</div>
						<div class="content typo" :class="{'privacy':!moment.published}" v-viewer v-else v-html="convertHTML(moment.content)"></div>
