<template>
	<div>
		<div class="ui padded attached segment m-padded-tb-large">
			...
			<div class="ui middle aligned mobile reversed stackable">
				<div class="ui grid m-margin-lr">
					...
					<div class="row m-padded-tb-small">
						<div class="ui horizontal link list m-center">
							<div class="item m-datetime">
								<i class="small calendar icon"></i><span>{{ blog.create_time | dateFormat('YYYY-MM-DD') }}</span>
							</div>
							<div class="item m-views">
								<i class="small eye icon"></i><span>{{ blog.views }}次观看</span>
							</div>
							<div class="item m-common-black">
								<i class="small pencil alternate icon"></i><span>{{ blog.words }}字</span>
							</div>
							<div class="item m-common-black">
								<i class="small hourglass half icon"></i><span>{{ blog.read_time }}分钟阅读</span>
							</div>
							<!-- 修改后的分类 -->
							<div class="item m-common-black">
								<router-link :to="`/category/${blog.category}`" class="ui" style="color:orange">
					...
					<!-- 文章Markdown正文 -->  <!-- style="padding-left:0rem;padding-right:0rem;" 自己加的样式，让正文更宽 -->
					<div class="typo js-toc-content m-padded-tb-small match-braces rainbow-braces" style="padding-left:0rem;padding-right:0rem;" v-viewer :class="{'m-big-fontsize':bigFontSize}" v-html="convertHTML(blog.content)"></div>
					<!--赞赏-->
						...
						<el-popover placement="top" width="220" trigger="click">
							<div class="ui orange basic label" style="width: 100%">
								<div class="image">
									...
									<img :src="$store.state.siteInfo.rewardImgUrl" alt="" class="ui rounded bordered image" style="width: 100%">
		...
		<div class="ui attached positive message">
			<ul class="list">
				<!-- <li v-if="blog_up">前一篇：<router-link :to="'/blog/' + blog_up.id">{{ blog_up.title }}</router-link> -->
				<!-- <li  v-if="blog_down">后一篇：<router-link :to="'/blog/' + blog_down.id">{{ blog_down.title }}</router-link></li> -->
				<li v-if="blog_up">前一篇：<a href="javascript:" @click.prevent="toBlog(blog_up)">{{ blog_up.title }}</a></li>
				<li v-if="blog_down">后一篇：<a href="javascript:" @click.prevent="toBlog(blog_down)">{{ blog_down.title }}</a></li>
...
<script>
	import {getBlogById, checkBlogPassword} from "@/api/blog";
		...
		methods: {
			getBlog(id = this.blogId) {
				...
				getBlogById(token, id).then(res => {
					if (res.code === 200){
						this.blog = res.blog
						this.blog_up = res.blog_up
						this.blog_down = res.blog_down
